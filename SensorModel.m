...
for id = 1:LMc
    ...
    if  q < 12.0           %%%If dist is shorter than 12, it can be found.
        
        ran = mvnrnd([q atan2(LM_Y(id)-X(2),LM_X(id)-X(1))-X(3)],Q);
        
        SenseData(id,1) = 1;    %if it's found, flag is set to 1
...
%         end
        SenseData(id,3) = ThetaRegulator(ran(2)); %%%Confind the theta to  +-pi
    end
